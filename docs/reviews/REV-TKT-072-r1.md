...
**반려 → started.** [블로커] 모바일 375px SVG 가로 스크롤 미작동 인정 — 구현자(PM 본인)가 수정 후 r2. [중요] RouteRow 미분리는 S5(TKT-075)로 이관. '예정 안내' 문구-동작 차이는 상시 라벨 방식을 정식 채택으로 문구 쪽을 수정(티켓 완료 기준 개정).

...
## r12 — PM 직접 제작 (2026-09-14, PO "난잡해 / 플랩 이상해, 기존 목업 심플함 섞어줘") — 자체 평가 동반

- **home.html r12b**: 링 위 환승역 **3개**(경험 동·학습놀이 남·운영 서) + 환승 홀 북. 각 환승역에서 **노선 1개**가 직선으로 나가고 노선 위에 서비스가 큰 역(문자 배지), 하위 화면은 작은 점(데스크톱만). 링에서 나가는 선 8→3, 십자 구도. 실측 390 overflow 0.
- **splash.html r12**: r4 의 심플 구성(다크 풀블리드, 큰 C 심볼 → 워드마크 → 한 줄 플랩 → 티커 → 진행선)에 r9 의 상·하 반쪽 플립만 얹음. 흰 카드·검은 보드 면 제거. 제작 중 결함 2건 자체 수정(그리드 justify 로 플랩 폭 0, 라이트 규칙 되돌림).
- **자체 평가**: 강점 — 노선도가 처음으로 "한 눈에 4층(사이트·분류·서비스·화면)"이 읽히면서 조용함. 스플래시는 PO 가 좋아한 r4 인상 + 좋아한 r9 동작의 합. 약점 — ①학습·놀이선이 남쪽으로 길어 모바일에서 링 아래 공간을 많이 씀(발견·취향 dim 이 화면 끝에 닿음) ②데스크톱은 십자가 정직해서 "지도"보다 "다이어그램" 인상 — 구현 시 링 안쪽에 얇은 도시 배경선 같은 질감을 더할 여지 ③환승역 3개만 남아 노선 문자(B·V…)가 링에서 사라짐 — 컨셉 단서는 큰 역 배지가 대신.
- 권고: r12b 형태 확정 시 D-015 를 "안 D: 순환선 + 환승역 3 + 노선 3"으로 갱신하고 111 기준으로.

## PM r10 판정 (2026-09-14) — **통과**

- writing-studio.html: 우측 패널 = 버튼 3(사진 첨부·H1/H2/H3·표 삽입), 미니 표·행/열 버튼 제거. 본문 인라인 표(hover: 우측 `+열`, 하단 `+행`, 셀 `⋯`), 데스크톱 중앙 컬럼, `← 블로그`, 390 overflow 0, script 0. 스펙 §6 개정판·TKT-105 r2 구현과 일치.
- 이로써 r1~r12 전 라운드 통과. TKT-102 종결 조건 = PO 최종 확인(홈 r12b·스플래시 r12).

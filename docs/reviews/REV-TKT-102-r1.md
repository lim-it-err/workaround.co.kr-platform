...
## PM r9 판정 (2026-09-14) — **통과 (PO 심볼 선택 대기)**

- `splash.html` 플립 CSS 개선: 셀 14칸 상·하 반쪽 접힘(rotateX/backface), 셀별 시차, 중간 프레임에서 반쪽 글자 정합 확인. 문구 3단계 `WORKING AROUND → MIND THE GAP → DOORS OPENING`, 티커 3회, 좌우 패널 없음, `환승 홀 →` 건너뛰기, 라이트 테마 대응. 390 overflow 0, script 0.
- `loop-symbol.html`(+ `design/proposals/2026-09-14-loop-symbol.html`) 3안 — A 돌아오는 길(닫힌 링+기점 점) / B 가벼운 궤도(가는 선·큰 기점) / C 열린 기점(기점 양옆 틈 — 문 열림 암시). 각 24/48/96px·상단바·파비콘 16px 예시, 근거 한 줄씩. **PM 권고 A**: 16px 에서도 한 노선+출발점이 읽히고 r8 노선도의 환승 홀 표기와 동형. C 는 컨셉이 가장 좋으나 16px 에서 끊긴 링으로 오독 위험 — 스플래시 대형 전용 변형으로 재활용 가능.
- 지적 없음. 심볼 확정 시 TKT-110 의 `SiteLoopSymbol.vue` 교체는 소형 후속 티켓.

## r11 — PM 직접 제작 판정 (2026-09-14) — **자체 평가 동반**

- **바뀐 것**: 링 9→모바일 11px 단색(주인공), 환승역 배지·이름은 링 안쪽, **모바일은 환승역 이름만·정류장은 점**(정류장 이름 데스크톱 전용), 지선 연장(정류장 간격 35~45), 구간명 링 안쪽 수평(모바일 숨김), 환승 홀 = C 심볼, 라이트 테마 토큰. 스플래시 보드 높이 한 줄+여백.
- **자체 평가**: 강점 — r8 대비 첫 시선이 링에 가고 모바일에서 이름 8개만 남아 "민들레" 인상 해소. 약점 — ①모바일에서 정류장 이름이 없어 "지선에 뭐가 있나"는 아래 목록에 의존(구현 시 정류장 탭 툴팁 권고) ②S 지선 정류장 4개가 하단 가장자리에 붙음(720 viewBox 여유 부족) ③D·P 스텁 라벨이 링 안쪽 구간명 자리와 경쟁 — 구간명을 모바일에서 숨겨 회피. 실측 390: overflow 0, 배지 r 21px, 라벨 최소 ≥11px(환승 홀 라벨 CSS 순서 결함 수정 후).
- 권고: 이 형태를 111 구현 기준으로. 디자이너는 r10(표 UX)·Advisor UX 검수로.

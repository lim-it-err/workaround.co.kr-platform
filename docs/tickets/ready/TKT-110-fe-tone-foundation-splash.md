...
2. 스플래시 — **현행 UI 기준(PO 2026-09-14)**: 배경·플랩보드 구조·**기존 split-flap 엔진(TKT-070/077) 그대로**. 바꾸는 것만: 문구 시퀀스 4단계 `WORKAROUND → WORKING AROUND → MIND THE GAP → DOORS OPENING`(엔진의 플립으로 전환), 티커 3회 교체, 배지→순환선 심볼(r9 확정안), 통계 타일·군더더기 행 제거. 목업 `splash.html` 의 CSS 플립·좌우 패널 연출은 **참고하지 않는다**(폐기). 정적 프레임(r9)만 참고.

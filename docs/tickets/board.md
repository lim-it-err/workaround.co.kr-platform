...
- `TKT-072` `P1` `chore` `진행 가능` UI 재구현 S2 - 환승 홀 노선도(시안 C) [시안 적용] — 수용 기준에 `design/ux-copy-audit-2026-08-16.md` §2 카피 원칙 포함
- `TKT-073` `P2` `chore` `진행 가능` UI 재구현 S3 - 블로그 사인 + 상태 배지 [시안 적용]
- `TKT-074` `P2` `chore` `진행 가능` UI 재구현 S4 - 시뮬/운영 페이지 사인 [시안 적용]
- `TKT-075` `P2` `chore` `진행 가능` UI 재구현 S5 - App.vue 컴포넌트 분해 [시안 적용]
- `TKT-076` `P3` `chore` `진행 가능` UI 재구현 S6 - 모바일 재배치 + 스튜디오 [시안 적용]
- `TKT-078` `P1` `v0.7.0` `v0.7.0 진행 시 가능` 화이트채플 아케이드 편입 마감 — 라우팅·매니페스트 (코드 subtree 흡수는 2026-08-16 완료, D-003~005)
- `TKT-079` `P2` `v0.7.0` `v0.7.0 진행 시 가능` advisor 디자인·프레임 통합 — 미션 화면을 모선 룩앤필로 (PO: "디자인 다른 건 백로그로", TKT-071 뒤 권장)
- `TKT-080` `P1` `v0.7.0` `v0.7.0 진행 시 가능` advisor 서비스 계약 마감 — /health·라우트·매니페스트·빌드 게이트

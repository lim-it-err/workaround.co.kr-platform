...
- 우선순위: P1 (PM 보드 배정 반영)
- 대상 버전: `chore`
- 상태: `backlog` — `blocked` (착수 전 담당·완료 범위 PM 확인 대기)
- 문서 상태: `작성완료`
- 진행 판정: `blocked/진행 불가` (2026-09-10 codex-1 착수 게이트 점검, 아래 질문 참조)
- 소유자 유형: `worker`
...
### 2026-09-10 codex-1 — 담당 및 완료 범위 충돌 확인

- 보드는 이 티켓을 `P1 [SIM] ready`, `design/sim-taxi-spec.md`의 택시 지도 작업으로 지정하고 work/runtime 명료화는 FE 후속 분할로 적고 있다. `docs/history/2026-08-17.md`의 "오전 심의 집행"에도 R1 승인 뒤 SIM ready로 바뀌었다는 근거가 있다.
- 반면 티켓 말미에는 "claude 직접 수행"이 남아 있고, 더 최근인 `docs/history/2026-09-08.md`에도 TKT-074/082를 "PM 직접 처리 예정"으로 적었다. 9월 9일 codex-1 인박스는 TKT-082만 명시적으로 재배정했으며 TKT-074 포인터는 없다. 최신 담당 결정을 어느 쪽으로 볼지 확정할 수 없다.
- 본문 완료 기준은 여전히 elevator/taxi/work/runtime/simhub 5개 페이지 개편이다. 현재 SIM 레인에서 work/runtime·공용 토큰/헤더를 수정할 수 없으며, 택시 지도만 완료하고 이 5개 페이지 기준을 충족했다고 선언할 수 없다. 기존 simhub 기준도 후속 D-012/TKT-101과 충돌하므로 되돌리지 않는다.
- **PM 질문**: 이 티켓을 codex-1의 택시 지도 단독 작업(`design/sim-taxi-spec.md` §2~4, 기존 TKT-082 코어 보존)으로 확정하고, 나머지 페이지 범위와 완료 기준은 별도 티켓으로 분리할지 확인해 주세요. PM 직접 수행을 유지한다면 보드 담당도 정렬해 주세요.
- 해제 조건: 담당과 수정 범위·완료 게이트가 티켓/보드에 일치하도록 PM이 확정할 것. 이번에는 착수 전 점검만 했으므로 backlog 위치에서 blocked로 두고 코드·스펙은 수정하지 않았다. TKT-082는 need_review이며 지도와 별개로 유지한다.
- 다음 FE 후보 TKT-075는 이 티켓(S4)을 명시적 선행 조건으로 요구하므로 먼저 착수하지 않았다. 빌드·브라우저 게이트 미실행, 커밋·push 없음.
...
## PO 추가 요구 (2026-08-17 — 이 티켓은 claude 직접 수행)

- **"각 기능별로 너무 뭐가 뭔지 모르겠어"** → S4 의 1차 목표를 스킨이 아니라 **기능 명료화**로 격상: 각 승강장 첫 화면에서 ①이게 뭐 하는 화면인지(역명판 요약 한 줄로) ②지금 무슨 일이 벌어지는지(핵심 수치·상태) ③내가 뭘 할 수 있는지(조작부)가 3초 안에 구분되게. 패널 재배치·병합·이름 재작성 포함.
- **"택시 시뮬레이터는 최소한 지도는 있어야겠지"** → 9구역을 표 안의 카드가 아니라 **3×3 지도형 그리드(미니 맵)**로: 구역 = 타일, 수요 강도 = 타일 톤, 차량 위치 = 마커, 이동 = 타일 간 애니메이션 또는 경로 표시. 요청/보상 패널은 지도 옆 보조로.
- 역명판 "← 환승 홀" 클릭 이동은 2026-08-17 선반영 완료 (StationHeader).

...
## 정적 자산 서비스 계약 (D-005, 2026-08-16)

빌드 산출물이 순수 정적 파일(HTML/CSS/JS)이라 자기 프로세스가 없는 서비스는, 위 필수 계약 대신 아래 4개로 계약을 만족한 것으로 본다.

- 빌드 명령과 산출물 경로 (자체 `README.md` 에 표로)
- reverse proxy(Caddy) 라우트 prefix
- `/health` — 정적 서버 컨테이너가 대신 응답한다 (개별 정적 서비스는 자기 헬스를 갖지 않는다)
- 자산 출처·권리 표기

첫 적용 사례: `services/arcade/whitechapel/` (TKT-028 게임 공간의 첫 입주자).

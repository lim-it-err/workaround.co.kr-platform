...
- `v0.5.0` 같은 다음 버전 가상 화면은 기존 검수 레일과 섞지 않고 `/test/v0-5-0` 아래에 둔다.
- `http://localhost:7000/test/v0-5-0/junction` 를 기본 진입점으로 두고, 아래 하위 승강장을 분리한다.
  - `http://localhost:7000/test/v0-5-0/taxi`
  - `http://localhost:7000/test/v0-5-0/ops`
  - `http://localhost:7000/test/v0-5-0/signals`
- 앞으로 디자인/구현 검수 문서에서 "테스트 경로"라고만 쓰면 기본적으로 `/test` 를 뜻하고, 차기 버전 프로토타입은 명시적으로 `/test/v0-5-0` 처럼 적는다.
...
- 차기 버전 더미나 디자인 실험은 `/` 경로를 덮지 않는다. 별도 `/test/...` 레일에서만 검토한다.
...
- `Sim Hub` 는 메인 허브와 실제 시뮬레이터 사이의 중간 환승면으로 두고, Elevator / Taxi 선택만 맡긴다.
...
- `v0.5.0` 택시 시뮬레이터는 메인 허브에서 별도 승강장으로 진입해야 하며, 구역 지도, 요청 흐름, 차량 상태, 리워드가 한 보드에서 연결돼 읽혀야 한다.
- `v0.6.0` `Blog District` 는 메인 허브 또는 Sim Hub 에서 분기하되, 공개 읽기와 Writing Studio 가 시뮬레이터와 다른 리듬으로 보여야 한다.
- 공개 글 상세는 폭을 제한한 문서형 읽기 레이아웃을 유지하고, Studio 는 편집/미리보기 분리 구조를 우선한다.
...
- `v0.3.1` 패치 이후에는 엘리베이터 대수 2~6 조절, car 내부 승객 dot 표현, 더 긴 position easing 을 함께 유지한다.
...
- `v0.5.0` 에서는 worker ownership summary, priority reaction hint, file-backed audit/persistence summary 를 함께 보여 주되, 실제 승인/종료 판정은 여전히 수동 구간임을 명확히 남긴다.

...
describe('연습 모드 로컬 상태', () => {
  ...
  it('데일리 키를 건드리지 않고 연습 전용 키에만 기록한다', async () => {
    localStorage.setItem('advisor.learner.v1', JSON.stringify({ seasonStats: { gains: [{ source: 'daily' }] } }))
    ...
    expect(JSON.parse(localStorage.getItem('advisor.learner.v1')).seasonStats.gains).toHaveLength(1)

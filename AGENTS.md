...
> **체제 변경 (D-001, 2026-08-16)**: PM은 이제 **Claude**다 (`CLAUDE.md` 참조). 이 저장소에서 Codex의 역할은 **구현 전담 개발자**다 — 티켓 구현, 테스트, 빌드 검증, 히스토리 기록. 티켓 발행·스펙 변경·커밋은 Claude 소관이며, 스펙이 모호하면 우회 구현하지 말고 티켓의 질문 섹션에 기록 후 `blocked`로 둔다. 기존 오케스트레이터/디자이너 레인 구성은 PO의 에이전트 재정산에 따라 추후 재개정 예정이다. 아래 원문 중 역할 서술은 이 체제 변경이 우선하고, **가드레일·문서 규칙·히스토리 규칙은 계속 유효하다.**
...
- 기능 페이지(엘리베이터/택시/Work Manager/블로그 등) 작업은 `docs/feature-definition.md` 의 페이지 책임과 기능 경계를 기준으로 삼는다.

## GitHub 문서 공개 규칙

- `docs/` 는 **공개 저장소에 그대로 올린다** (PO 2026-09-11 "docs는 공개해도 돼", Q-014→D-014-4). 따라서 문서에도 코드와 같은 기준이 적용된다: 비밀값·개인 식별 정보·외부인 실명은 쓰지 않고, PO 발언 인용은 업무 맥락에 한정한다.
